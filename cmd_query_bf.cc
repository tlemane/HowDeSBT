...
void QueryBFCommand::usage
   (std::ostream& s,
	...
	s << "                       			query file" << endl;
	s << "  --repart=<repartitionfilename>  repartition file generated by kmtricks. File is named" << endl;
	s << "                       			storage/partition_storage_gatb/minimRepart.minimRepart" << endl;
   ...
   (int		_argc,
	...
	for (int argIx=0 ; argIx<argc ; argIx++)
		...
		if (is_prefix_of (arg, "--repart=")){
			repart_filename = strip_blank_ends(argVal);
		}
...
int QueryBFCommand::execute()
	...
	for (const auto& bfFilename : bfFilenames)
		{
		BloomFilter* bf = BloomFilter::bloom_filter(bfFilename, repart_filename);
		bf->load();
